...
public class ProductosController {
        ...
        if (iden==null || iden.isEmpty()) {
            ...
            try(Statement stm = con.getCon().createStatement()){
                ...
                if(rta.next()){
                    ...
                    txtEntrada.setText("No se encontro registro que coincida");
            }
            con.desconectar();
        }
    ...
    void clickBorrar(MouseEvent event) throws SQLException {
        ...
        query = "DELETE FROM `inventario`.`productos` WHERE (`codigo` = '"+codigoProducto+"')";
        ...
        if(res!=0){
            txtEntrada.setText("Registro Borrado con exito");
        }else
            txtEntrada.setText("Error al Borrar registro");
        restaurarDatos();
        }
        con.desconectar();
    }
    ...
    void clickGuardar(MouseEvent event) throws SQLException {
    	...
    	System.out.println("funciona guardar!!");
        ...
        if(nom2==null || nom2.isEmpty())
            ...
            String query1 = "insert into productos (nombre,pCompra,pVenta,cBodega,cMinima,cMaxPer)values ('"+nom2+"','"+Double.parseDouble(Pcom2)+"','"+Double.parseDouble(Pven2)+"','"+Integer.parseInt(Cbo2)+"','"+Integer.parseInt(Cmin2)+"','"+Integer.parseInt(Cmax2)+"')";
            con.conectar();
            try (Statement stm = con.getCon().createStatement()){
